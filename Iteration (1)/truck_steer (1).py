...
def main():
    ...
    while running:
        ...
        # At zero steer the tyre centre is directly ahead of the KPI by T_D/2
        # (truck +Y). When steered by angle theta the offset vector rotates:
        #   truck_dx =  (T_D/2)*sin(theta)  → screen +x
        #   truck_dy =  (T_D/2)*cos(theta)  → screen -y  (screen y is flipped)
        half_td = DIMS["T_D"] / 2

        la_r = math.radians(left_angle)
        fl_cx = kpi_left[0]  + m2p( half_td * math.sin(la_r))
        fl_cy = kpi_left[1]  - m2p( half_td * math.cos(la_r))
        draw_tyre(screen, fl_cx, fl_cy, left_angle,
                  DIMS["T_W"], DIMS["T_D"], color=STEER_COLOR)

        ra_r = math.radians(right_angle)
        fr_cx = kpi_right[0] + m2p( half_td * math.sin(ra_r))
        fr_cy = kpi_right[1] - m2p( half_td * math.cos(ra_r))
