...
FPS           = 60
SCALE         = 44          # pixels per metre (scaled down a tad)
...
def main():
    ...
    while running:
        ...
        W, H = screen.get_size()
        ox = W / 2
        oy = H / 2 + m2p(DIMS["W_B"] / 2) - 30
        screen.fill(BG)

        # light grid
        for gx in range(0, W, 40):
            pygame.draw.line(screen, GRID_COLOR, (gx, 0), (gx, H), 1)
        for gy in range(0, H, 40):
            pygame.draw.line(screen, GRID_COLOR, (0, gy), (W, gy), 1)
        ...
        # ── Draw front tyres (steered about KPI) ──────────────────────────────
        # The stub-axle length is the lateral distance from KPI to tyre centre
        # when straight ahead (angle=0): stub = (T_F - D) / 2
        # At angle=0 the offset from KPI to tyre centre is purely lateral
        # (left: -stub in truck-X, right: +stub in truck-X).
        # When steered, that whole vector rotates by the steer angle.
        # Truck-space: right=+x, forward=+y  →  screen: right=+sx, up=-sy
        #
        # Left tyre: base offset = (-stub, 0)
        #   rotated by left_angle (CCW = positive steer left):
        #   dx_truck = -stub*cos(a),  dy_truck = -stub*(-sin(a)) = stub*sin(a)  [CCW rot]
        #   Wait — standard 2D CCW rotation of vector (vx,vy) by angle a:
        #     vx' = vx*cos(a) - vy*sin(a)
        #     vy' = vx*sin(a) + vy*cos(a)
        #   Left base vector: vx=-stub, vy=0
        #     vx' = -stub*cos(a)
        #     vy' = -stub*sin(a)
        #   Right base vector: vx=+stub, vy=0
        #     vx' = +stub*cos(a)
        #     vy' = +stub*sin(a)
        stub = (DIMS["T_F"] - DIMS["D"]) / 2

        la_r = math.radians(left_angle)
        fl_dx =  -stub * math.cos(la_r)   # truck-x
        fl_dy =  -stub * math.sin(la_r)   # truck-y (forward)
        fl_cx = kpi_left[0] + m2p(fl_dx)
        fl_cy = kpi_left[1] - m2p(fl_dy)  # screen-y flipped
        # stub axle line from KPI to tyre centre
        pygame.draw.line(screen, AXLE_COLOR, (int(kpi_left[0]), int(kpi_left[1])),
                         (int(fl_cx), int(fl_cy)), 4)
        draw_tyre(screen, fl_cx, fl_cy, -left_angle,
                  DIMS["T_W"], DIMS["T_D"], color=STEER_COLOR)

        ra_r = math.radians(right_angle)
        fr_dx = +stub * math.cos(ra_r)    # truck-x
        fr_dy = +stub * math.sin(ra_r)    # truck-y (forward)
        fr_cx = kpi_right[0] + m2p(fr_dx)
        fr_cy = kpi_right[1] - m2p(fr_dy) # screen-y flipped
        pygame.draw.line(screen, AXLE_COLOR, (int(kpi_right[0]), int(kpi_right[1])),
                         (int(fr_cx), int(fr_cy)), 4)
        draw_tyre(screen, fr_cx, fr_cy, -right_angle,
                  DIMS["T_W"], DIMS["T_D"], color=STEER_COLOR)

        # ── Infinite dashed lines coincident with stub axles ─────────────────
        # Direction vector is simply (wheel_centre - KPI), extended to infinity
        def draw_infinite_dashed_from_dir(surface, px, py, dx, dy, color, dash=10, gap=4, width=1):
            ...
            for sign in (1, -1):
                ...
                while t < max_dist:
                    x1 = px + sign * ux * t
                    y1 = py + sign * uy * t
                    x2 = px + sign * ux * (t + dash)
                    y2 = py + sign * uy * (t + dash)
                    pygame.draw.line(surface, color,
                                     (int(x1), int(y1)), (int(x2), int(y2)), width)
                    t += dash + gap

        dash_color = (100, 100, 100)
        # Rear axle: purely horizontal (direction = (1, 0))
        draw_infinite_dashed_from_dir(screen, rear_mid[0], rear_mid[1],
                                      1, 0, dash_color)
        # Front stubs
        draw_infinite_dashed_from_dir(screen, fl_cx, fl_cy,
                                      fl_cx - kpi_left[0], fl_cy - kpi_left[1], dash_color)
        draw_infinite_dashed_from_dir(screen, fr_cx, fr_cy,
                                      fr_cx - kpi_right[0], fr_cy - kpi_right[1], dash_color)
